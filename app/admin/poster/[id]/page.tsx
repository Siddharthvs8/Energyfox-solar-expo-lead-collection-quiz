...
import { MAX_DISCOUNT, PRIZES, QUESTIONS_PER_QUIZ } from "@/lib/prizes";
    ...
    <main className="min-h-dvh bg-navy-100 py-8 print:bg-white print:p-0">
      ...
      <article className="relative mx-auto flex h-[297mm] w-[210mm] flex-col items-center overflow-hidden bg-navy-950 px-[16mm] pt-[16mm] pb-[12mm] text-center text-white shadow-2xl print:shadow-none">
        ...
        <div className="relative flex flex-1 flex-col items-center">
          ...
          <p className="mt-[9mm] text-[4.2mm] font-bold tracking-[0.35em] text-sun-300 uppercase">Solar Quiz Challenge</p>
          <h1 className="mt-[3mm] font-display text-[15mm] leading-[1.05] font-bold tracking-tight">
            Scan &amp; win up to
            <br />
            <span className="text-sun-400">{MAX_DISCOUNT}% OFF</span>
          </h1>
          <p className="mt-[4mm] text-[5.2mm] text-white/75">
            Answer {QUESTIONS_PER_QUIZ} quick questions about solar energy
          </p>
          ...
          <ul className="mt-auto grid w-full grid-cols-3 gap-[4mm]">
            {PRIZES.map((prize, i) => (
              ...
              >
                <p className="text-[3.4mm] font-semibold tracking-wide text-white/65 uppercase">
                  {prize.score}/{QUESTIONS_PER_QUIZ} correct
                </p>
                <p className={`mt-[1mm] font-display text-[11mm] leading-none font-bold ${i === 0 ? "text-sun-400" : ""}`}>
                  {prize.discount}%
                </p>
                <p className="mt-[1mm] text-[3.2mm] font-bold tracking-[0.25em] text-sun-300 uppercase">off</p>
              </li>
            ))}
          </ul>
        </div>
        ...
        <footer className="relative mt-[7mm] flex w-full items-center justify-between border-t border-white/15 pt-[4mm] text-[3.4mm] text-white/55">
          <span>One entry per mobile number</span>
